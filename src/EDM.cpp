...
void EDM::print_edm() const {
    if (_initialized) {
        ...
        vector<tuple<double, string>> orderedDiagrams = _allDiagrams_EDM;
...
void EDM::print_amm() const {
    if (_initialized) {
        ...
        vector<tuple<double, string>> orderedDiagrams = _allDiagrams_AMM;
        sort(orderedDiagrams.begin(), orderedDiagrams.end(), sortTuples);

        Table tab(2);
        tab.set_frame_style("-", "|");
        tab.set_title("Muon AMM (10 largest diagrams)");
        tab.set_frame_color(BLUE);

        using l = vector<string>;
        tab.add_row(l{"Diagram", "|a_mu| [n/a]"}, true);
        for (int i = 0; i < 11; ++i)
            tab.add_row(l{std::get<1>(orderedDiagrams[i]),
                          stringAuto(std::get<0>(orderedDiagrams[i]))});

        tab.add_row(l{"Total", stringAuto(_amu[0])}, true);
        tab.print();
        cout << "\n";
    } else
        printf("AMM: Failed to be initialized:(\n");
}

SLHA_BLOCK EDM::get_slha_block_edm() const {
...
SLHA_BLOCK EDM::get_slha_block_amm() const {
    SLHA_BLOCK block("AMM", "Anomalous magnetic moment of muon");
    if (_initialized) {
        block.add_parameter(1, _amu[0], "a_mu");
    } else
        block.add_parameter(1, -1., "AMM not initialized");
    return block;
}
...
std::vector<double> EDM::get_amu() const {
    if (!_initialized) {
        std::vector<double> zeros;
        zeros.resize(38, 0.);
        return zeros;
...
void EDM::compute_wilson_coefficients() {
    ...
    for (int h = 0; h < 3; ++h) {
		Eigen::Matrix3cd coupls = _thdms.back().get_hll(h+1);
        for (int f = 2; f < 3; ++f) {
            if (_runningTHDM) {
                set_parameters(_thdms[h + 2]);
            }
			// 1-loop contribution with FCNC
			double oneLoop_EDM = 0.;
			double oneLoop_AMM = 0.;
			for(int l_loop = 0; l_loop < 3; ++l_loop){
				oneLoop_EDM += de_oneLoop_integral(coupls(0,l_loop), coupls(l_loop,0), _ml[0], _ml[l_loop], _mh[h]);
				oneLoop_AMM += amu_oneLoop_integral(coupls(1,l_loop), coupls(l_loop,1), _ml[1], _ml[l_loop], _mh[h]);
			}
			_oneLoop_EDM[f][h] =
				oneLoop_EDM;
			_oneLoop_AMM[f][h] =
            ...
            _barrZee_AMM_gh_b[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::nC * Global::Qf[f] * Global::Qf[1] * Global::Qf[1] *
                 _e * _e / (64. * std::pow(M_PI, 4.))) *
                (fz(zbi) * _c[5][h] * _c[3 * f + 1][h] -
                 gz(zbi) * _cTilde[5][h] * _cTilde[3 * f + 1][h]);

            // 2-loop Barr-Zee gamma-h (tau loop)
            double ztaui = _ml[2] * _ml[2] / (_mh[h] * _mh[h]);
            _barrZee_EDM_gh_tau[f][h] =
                (-Global::Qf[f] * Global::Qf[2] * Global::Qf[2] * _e * _e /
                 (64. * std::pow(M_PI, 4.))) *
                (fz(ztaui) * _c[8][h] * _cTilde[3 * f][h] +
                 gz(ztaui) * _cTilde[8][h] * _c[3 * f][h]);
            _barrZee_AMM_gh_tau[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::Qf[f] * Global::Qf[2] * Global::Qf[2] * _e * _e /
                 (64. * std::pow(M_PI, 4.))) *
                (fz(ztaui) * _c[8][h] * _c[3 * f + 1][h] -
                 gz(ztaui) * _cTilde[8][h] * _cTilde[3 * f + 1][h]);
                
            // 2-loop Barr-Zee Z-h (b loop)
            double mbMZ = _mdn[2] * _mdn[2] / (Global::mZ * Global::mZ);
            _barrZee_EDM_Zh_b[f][h] =
                (-Global::nC * Global::Qf[f] * _gZff[f] * _gZff[1] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(zbi, mbMZ) * _c[5][h] * _cTilde[3 * f][h] +
                 gTilde(zbi, mbMZ) * _cTilde[5][h] * _c[3 * f][h]);
            _barrZee_AMM_Zh_b[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::nC * Global::Qf[f] * _gZff[f] * _gZff[1] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(zbi, mbMZ) * _c[5][h] * _c[3 * f + 1][h] -
                 gTilde(zbi, mbMZ) * _cTilde[5][h] * _cTilde[3 * f + 1][h]);

            // 2-loop Barr-Zee Z-h (tau loop)
            double mtauMZ = _ml[2] * _ml[2] / (Global::mZ * Global::mZ);
            _barrZee_EDM_Zh_tau[f][h] =
                (-Global::Qf[f] * _gZff[f] * _gZff[2] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(ztaui, mtauMZ) * _c[8][h] * _cTilde[3 * f][h] +
                 gTilde(ztaui, mtauMZ) * _cTilde[8][h] * _c[3 * f][h]);
            _barrZee_AMM_Zh_tau[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::Qf[f] * _gZff[f] * _gZff[2] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(ztaui, mtauMZ) * _c[8][h] * _c[3 * f + 1][h] -
                 gTilde(ztaui, mtauMZ) * _cTilde[8][h] * _cTilde[3 * f + 1][h]);
            ...
            _barrZee_EDM_gh_W[f][h] =
                (Global::Qf[f] * _e * _e / (256. * std::pow(M_PI, 4.))) *
                (((6. + 1. / zWi) * fz(zWi) + (10. - 1. / zWi) * gz(zWi)) *
                 _a[h] * _cTilde[3 * f][h]);
            _barrZee_AMM_gh_W[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::Qf[f] * _e * _e / (256. * std::pow(M_PI, 4.))) *
                (((6. + 1. / zWi) * fz(zWi) + (10. - 1. / zWi) * gz(zWi)) *
                 _a[h] * _c[3 * f + 1][h]);
                
            // 2-loop Barr-Zee Z-h (W loop)
            _barrZee_EDM_Zh_W[f][h] =
                ...
                (((6. - _secW * _secW + (2. - _secW * _secW) / (2. * zWi)) *
                  ...
                  (10. - 3. * _secW * _secW -
                   (2. - _secW * _secW) / (2. * zWi)) *
                      gTilde(zWi, _cW * _cW)) *
                 _a[h] * _cTilde[3 * f][h]);
            _barrZee_AMM_Zh_W[f][h] =
                ((2. * Global::mmuon) / _e) * (_gZff[f] * _e * _cotW / (256. * std::pow(M_PI, 4.))) *
                (((6. - _secW * _secW + (2. - _secW * _secW) / (2. * zWi)) *
                      fTilde(zWi, _cW * _cW) +
                  (10. - 3. * _secW * _secW -
                   (2. - _secW * _secW) / (2. * zWi)) *
            ...
            _barrZee_EDM_gh_t[f][h] =
                (-Global::nC * Global::Qf[f] * Global::Qf[0] * Global::Qf[0] *
                 _e * _e / (64. * std::pow(M_PI, 4.))) *
                (fz(zti) * _c[2][h] * _cTilde[3 * f][h] +
                 gz(zti) * _cTilde[2][h] * _c[3 * f][h]);
            _barrZee_AMM_gh_t[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::nC * Global::Qf[f] * Global::Qf[0] * Global::Qf[0] *
                 _e * _e / (64. * std::pow(M_PI, 4.))) *
                (fz(zti) * _c[2][h] * _c[3 * f + 1][h] -
                 gz(zti) * _cTilde[2][h] * _cTilde[3 * f + 1][h]);
                
            // 2-loop Barr-Zee Z-h (t loop)
            double mtMZ = _mup[2] * _mup[2] / (Global::mZ * Global::mZ);
            _barrZee_EDM_Zh_t[f][h] =
                (-Global::nC * Global::Qf[f] * _gZff[f] * _gZff[0] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(zti, mtMZ) * _c[2][h] * _cTilde[3 * f][h] +
                 gTilde(zti, mtMZ) * _cTilde[2][h] * _c[3 * f][h]);
            _barrZee_AMM_Zh_t[f][h] =
                ((2. * Global::mmuon) / _e) * (-Global::nC * Global::Qf[f] * _gZff[f] * _gZff[0] /
                 (64. * std::pow(M_PI, 4.))) *
                (fTilde(zti, mtMZ) * _c[2][h] * _c[3 * f + 1][h] -
                 gTilde(zti, mtMZ) * _cTilde[2][h] * _cTilde[3 * f + 1][h]);
            ...
            _barrZee_AMM_Wh_h[f][h] =
                ((2. * Global::mmuon) / _e) * (sf / (512 * std::pow(M_PI, 4.))) *
                (_e * _e * _a[h] * _c[3 * f + 1][h] *
                     I4(_mh[h], _mh[3], _mh[3], Global::mW, Global::mZ) /
                     (2. * _sW * _sW) -
                 I5(_mh[h], _mh[3], _mh[3], Global::mW, Global::mZ) *
                     _lambdaBar[h] * _c[3 * f + 1][h]);
        }
    }
    ...
    for (int f = 2; f < 3; ++f) {
        ...
        _barrZee_AMM_Wh_tb[f] =
            ((2. * Global::mmuon) / _e) * ((Global::nC * (_e * _e / (4. * M_PI)) * std::norm(_VCKM(2, 2))) /
             (128. * pow(M_PI, 3) * _sW * _sW * (mHc2 - mw2))) *
            (real(conj(_rL11) * conj(_rU33)) * Global::mt * Global::Qf[0] *
                 (Gi(1, o1_mH, o2_mH) - Gi(1, o1_mW, o2_mW)) +
             real(conj(_rL11) * conj(_rU33)) * Global::mt * Global::Qf[1] *
                 (Gi(2, o1_mH, o2_mH) - Gi(2, o1_mW, o2_mW)) +
             real(conj(_rL11) * _rD33) * Global::mb * Global::Qf[0] *
                 (Gi(3, o1_mH, o2_mH) - Gi(3, o1_mW, o2_mW)) +
             real(conj(_rL11) * _rD33) * Global::mb * Global::Qf[1] *
                 (Gi(4, o1_mH, o2_mH) - Gi(4, o1_mW, o2_mW)));
    }
...
void EDM::save_diagrams() {
	...
	static const double corr = 2. * Global::mmuon / (Global::v * Global::v);
    static const double dim =
        GeVinvToCm * 2. * Global::me / (Global::v * Global::v);
    for (int h = 0; h < 3; ++h) {
		_allDiagrams_EDM.push_back(std::make_tuple(GeVinvToCm * _oneLoop_EDM[2][h],
											   "1_loop" + to_string(h + 1)));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_gh_t[2][h],
                                               "gh" + to_string(h + 1) + "_t"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_gh_b[2][h],
                                               "gh" + to_string(h + 1) + "_b"));
        _allDiagrams_EDM.push_back(std::make_tuple(
            dim * _barrZee_EDM_gh_tau[2][h], "gh" + to_string(h + 1) + "_tau"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_Zh_t[2][h],
                                               "Zh" + to_string(h + 1) + "_t"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_Zh_b[2][h],
                                               "Zh" + to_string(h + 1) + "_b"));
        _allDiagrams_EDM.push_back(std::make_tuple(
            dim * _barrZee_EDM_Zh_tau[2][h], "Zh" + to_string(h + 1) + "_tau"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_gh_W[2][h],
                                               "gh" + to_string(h + 1) + "_W"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_Zh_W[2][h],
                                               "Zh" + to_string(h + 1) + "_W"));
        _allDiagrams_EDM.push_back(std::make_tuple(
            dim * _barrZee_EDM_gh_Hc[2][h], "gh" + to_string(h + 1) + "_Hc"));
        _allDiagrams_EDM.push_back(std::make_tuple(
            dim * _barrZee_EDM_Zh_Hc[2][h], "Zh" + to_string(h + 1) + "_Hc"));
        _allDiagrams_EDM.push_back(std::make_tuple(dim * _barrZee_EDM_Wh_h[2][h],
                                               "Wh_h" + to_string(h + 1) + ""));

		_allDiagrams_AMM.push_back(std::make_tuple(_oneLoop_AMM[2][h],
											   "1_loop" + to_string(h+1)));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_gh_t[2][h],
                                               "gh" + to_string(h + 1) + "_t"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_gh_b[2][h],
                                               "gh" + to_string(h + 1) + "_b"));
        _allDiagrams_AMM.push_back(std::make_tuple(
            corr * _barrZee_AMM_gh_tau[2][h], "gh" + to_string(h + 1) + "_tau"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_Zh_t[2][h],
                                               "Zh" + to_string(h + 1) + "_t"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_Zh_b[2][h],
                                               "Zh" + to_string(h + 1) + "_b"));
        _allDiagrams_AMM.push_back(std::make_tuple(
            corr * _barrZee_AMM_Zh_tau[2][h], "Zh" + to_string(h + 1) + "_tau"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_gh_W[2][h],
                                               "gh" + to_string(h + 1) + "_W"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_Zh_W[2][h],
                                               "Zh" + to_string(h + 1) + "_W"));
        _allDiagrams_AMM.push_back(std::make_tuple(
            corr * _barrZee_AMM_gh_Hc[2][h], "gh" + to_string(h + 1) + "_Hc"));
        _allDiagrams_AMM.push_back(std::make_tuple(
            corr * _barrZee_AMM_Zh_Hc[2][h], "Zh" + to_string(h + 1) + "_Hc"));
        _allDiagrams_AMM.push_back(std::make_tuple(corr * _barrZee_AMM_Wh_h[2][h],
                                               "Wh_h" + to_string(h + 1) + ""));
        
    }
    _allDiagrams_EDM.push_back(
        std::make_tuple(GeVinvToCm * _barrZee_EDM_Wh_tb[2], "Wh_tb"));
    
    _allDiagrams_AMM.push_back(
        std::make_tuple(_barrZee_AMM_Wh_tb[2], "Wh_tb"));
}
void EDM::compute_electron_edm() {
    _de.clear();
    double electron_edm = 0.;
    for (auto &diagram : _allDiagrams_EDM) electron_edm += std::get<0>(diagram);

    _de.push_back(std::abs(electron_edm));
    for (auto &diagram : _allDiagrams_EDM) _de.push_back(std::get<0>(diagram));
}

void EDM::compute_muon_amm() {
    _amu.clear();
    double muon_amm = 0.;
    for (auto &diagram : _allDiagrams_AMM) muon_amm += std::get<0>(diagram);

    _amu.push_back(muon_amm);
    for (auto &diagram : _allDiagrams_AMM) _amu.push_back(std::get<0>(diagram));
}
...
//1-loop integral functions

//the denominator of the 1-loop integrals, where x is the outer integral argument and y is the inner integral argument
//mf is the given lepton's mass, and mh_loop is the mass of the loop-boson
double oneLoop_divisor(double x, double y, double mf,double mh_loop){
        double divis = mf * mf * (y * (y - x) + 1 - y) + mh_loop * mh_loop * y;
        return divis;
}

//the numerator for the 1-loop anomalous muon magnetic moment integral
//y is the argument of the inner integral, yukawa_to is the coupling to the intermediary lepton from the muon,
//yukawa_from is the coupling from the intermediary lepton to the muon, ml_loop is the mass of the intermediary lepton
double amu_oneLoop_dividend(double y, std::complex<double> yukawa_to, std::complex<double> yukawa_from, double ml_loop){
	std::complex<double> yukawa_to_conj = std::conj(yukawa_to);
    double divid = (Global::mmuon / (8 * M_PI * M_PI)) * (y*(y-1)*std::real(yukawa_to_conj*yukawa_from) * Global::mmuon + (y-1)*(std::real(yukawa_to * yukawa_from)) * ml_loop);
    return divid;
}

//the numerator for the 1-loop electron dipole moment integral
//y is the argument of the inner integral, yukawa_to is the coupling to the intermediary lepton from the electron,
//yukawa_from is the coupling from the intermediary lepton to the electron, ml_loop is the mass of the intermediary lepton
double de_oneLoop_dividend(double y, std::complex<double> yukawa_to, std::complex<double> yukawa_from, double ml_loop){
	std::complex<double> yukawa_to_conj = std::conj(yukawa_to);
    double divid = (1 / (16 * M_PI * M_PI)) * (y*(y-1)*std::imag(yukawa_to_conj*yukawa_from) * Global::me + (y-1)*(std::imag(yukawa_to * yukawa_from)) * ml_loop);
    return divid;
}

//the total argument of the inner integral of a_mu
double amu_oneLoop_integrand(double x, double y, std::complex<double> yukawa_to, std::complex<double> yukawa_from, double mf, double mf_loop, double mh_loop){
    double val = (amu_oneLoop_dividend(y,yukawa_to,yukawa_from,mf_loop) / oneLoop_divisor(x,y,mf,mh_loop));
    return val;
}

//the total argument of the inner integral of d_e
double de_oneLoop_integrand(double x, double y, std::complex<double> yukawa_to, std::complex<double> yukawa_from, double mf, double mf_loop, double mh_loop){
    double val = (de_oneLoop_dividend(y,yukawa_to,yukawa_from,mf_loop) / oneLoop_divisor(x,y,mf,mh_loop));
    return val;
}

//two dimensional trapezoidal rule for the 1-loop a_mu integral. takes the 1-dim trapezoidal rule for each step of the inner integral from 0 to x 
//note that resolution is constant, meaning each evaluation of the inner integral has more steps
double amu_oneLoop_integral(std::complex<double> yukawa_to, std::complex<double> yukawa_from, double mf, double mf_loop, double mh_loop){
	int xsteps = 200; //nr of steps in the outer integral
	int ysteps = 100; //nr of steps in the inner integral per step in the outer integral
	double amu = 0; //the one-loop a_mu contribution
	double xstepsize = 1./xsteps; //how long a single step in the outer integral is
	double ystepsize = 1./(xsteps * ysteps); //how long a single in the inner integral, which goes from 0 to x is. note that the resolution is constant, regardless of which x-step the inner integral is on
	amu += ystepsize * (amu_oneLoop_integrand(0., 0., yukawa_to, yukawa_from, mf, mf_loop, mh_loop) * ((1. / 2.) + xstepsize));
	for(int xstep = 1; xstep <= xsteps; ++xstep){ //outer integral
		double x = 0. + xstep * xstepsize;
		for(int ystep = 1; ystep <= ysteps * xstep; ++ystep){//inner integral. evaluates the integral divison with "ysteps * xstep" steps for each x-step
			double y = 0. + ystep * ystepsize;
			amu += (xstepsize * ystepsize) * amu_oneLoop_integrand(x, x * y, yukawa_to, yukawa_from, mf, mf_loop, mh_loop);
		}
	}
	return amu;
}

//two dimensional trapezoidal rule for the 1-loop d_e integral. takes the 1-dim trapezoidal rule for each step of the inner integral from 0 to x 
//note that resolution is constant, meaning each evaluation of the inner integral has more steps
double de_oneLoop_integral(std::complex<double> yukawa_to, std::complex<double> yukawa_from, double mf, double mf_loop, double mh_loop){
	int xsteps = 200; //nr of steps in the outer integral
	int ysteps = 100; //nr of steps in the inner integral per step in the outer integral
	double de = 0; //the one-loop a_mu contribution
	double xstepsize = 1./xsteps; //how long a single step in the outer integral is
	double ystepsize = 1./(xsteps * ysteps); //how long a single in the inner integral, which goes from 0 to x is. note that the resolution is constant, regardless of which x-step the inner integral is on
	de += ystepsize * (de_oneLoop_integrand(0., 0., yukawa_to, yukawa_from, mf, mf_loop, mh_loop) * ((1. / 2.) + xstepsize));
	for(int xstep = 1; xstep <= xsteps; ++xstep){//outer integral
		double x = 0. + xstep * xstepsize;
		for(int ystep = 1; ystep <= ysteps * xstep; ++ystep){//inner integral. evaluates the integral divison with "ysteps * xstep" steps for each x-step
			double y = 0. + ystep * ystepsize;
			de += (xstepsize * ystepsize) * de_oneLoop_integrand(x, x * y, yukawa_to, yukawa_from, mf, mf_loop, mh_loop);
		}
	}
	return de;
}
